...
> **Restored from the archived predecessor repo** — last modified there 2026-07-19.
...
|---|---|
| **Fired and completed; archived to `3-COMPLETED` 2026-07-19** (renamed from the ad-hoc `CLIO-DURABLE-MARATHON/` to the dated `MARATHON-<date>-<slug>/` convention). Both phases approved (agy), both `swift build` gates green. Post-run verification ran the exporter on `/bin/bash` 3.2 and found the reconciliation array aborted with "unbound variable" (empty-array under `set -u`) — fixed with the `${arr[@]+…}` guard and re-verified (idempotency, state-delete, reconciliation, dry-run all pass). 48 swift tests pass. | **Operator litmus still outstanding** — install the updated exporter on both machines and watch a real conflict reconcile. Archived ahead of it deliberately; the code is merged to `development` via PR #162. |

## Plan

- **Parent plan:** [CLIO-DURABLE-IDEMPOTENT-WRITES.md](../../1-INBOX/CLIO-DURABLE-IDEMPOTENT-WRITES.md)
- **Phases:** [P1](https://github.com/Hypercart-Dev-Tools/rebalance-OS/blob/development/PROJECT/3-COMPLETED/MARATHON-2026-07-19-CLIO-DURABLE/briefs/p1-idempotent-append.md) (idempotent append + Swift reader) → [P2](https://github.com/Hypercart-Dev-Tools/rebalance-OS/blob/development/PROJECT/3-COMPLETED/MARATHON-2026-07-19-CLIO-DURABLE/briefs/p2-conflict-reconciliation.md) (reconciliation, `depends_on` P1)
...
.xyz/relay-automation/marathon.sh \
  --plan PROJECT/3-COMPLETED/MARATHON-2026-07-19-CLIO-DURABLE/MARATHON.yaml \
