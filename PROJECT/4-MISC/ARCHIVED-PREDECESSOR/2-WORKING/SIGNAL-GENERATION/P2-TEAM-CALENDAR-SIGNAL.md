...
> **Restored from the archived predecessor repo** — last modified there 2026-06-23.
...
> GIT-PULSE-REFACTOR.md. That refactor
...
- [x] One bounded read: `sync_calendar(calendar_id=<matt id>, days_back=14, days_forward=2)` into the live `calendar_events` table → **122 events stored**, `calendar_id`-tagged. Reversible (`DELETE WHERE calendar_id=<matt>`). Script: temp/phase0_sync_matt.py.
- [x] Confirm we now have both `primary` (584) and `<matt>` (122) rows for the same days.

### 0b. Build the A/B harness — temp/ab_team_signal.py
...
SIGNAL-GENERATION/REFERENCES/) — found largely
