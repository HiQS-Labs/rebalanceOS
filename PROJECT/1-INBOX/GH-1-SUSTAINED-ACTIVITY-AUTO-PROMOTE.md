---
gh_issue: 1
source: https://github.com/HiQS-Suite/rebalanceOS/issues/1
title: "GH-1 sustained-activity auto-promotion"
status: "Inbox — Codex/agy consult found a blocking design flaw; revised, needs re-triage"
created: 2026-08-16
updated: 2026-08-16
owner: noel
doc_type: feature
related: "Hypercart-Dev-Tools/rebalance-OS#306 (original ticket, moved here — that repo is retiring, see #291)"
goal: >
...
# GH-1 — Sustained-Activity Auto-Promotion

> Moved from `Hypercart-Dev-Tools/rebalance-OS#306` / `PROJECT/1-INBOX/GH-306-SUSTAINED-ACTIVITY-AUTO-PROMOTE.md`
> in the old repo — that repo is being retired per its own consolidation plan (#291 there).
> Content unchanged except renumbering; development continues here.

## Status

| What was just completed | What's next |
|---|---|
| Consult (Codex + agy, relay-xyz `consult.sh`) on the v1.0 draft — full transcripts on the operator's machine at `relay-system/2026-08-16/gh306-consult-085839/` in the old repo checkout (not committed to either repo's git history; `relay-system/` is gitignored in both). Both advisors independently converged: proposed design has a verified Blocker (burst-guard is a no-op) and a verified regression (drops cloud-agent commit credit). Plan revised below; ratings bumped 2/2/2 → 3/3/3 per both advisors' independent convergence. | Re-triage (clear `ratings_provisional` on the revised design), then Phase 0 spike below before implementation |
...
- **New visibility surface.** `log_project_auto_promoted()` already writes into the auth log and
  renders as a notice on the `/auth-log` System Log page
...
- **Type-diversity requirement** (e.g. "must include at least one non-commit action"). v1.0 treats
  ...
  they clear the 20h burst-guard gate below. Revisit only if that gate proves insufficient in
  practice.
...
source before being folded in (not taken on trust).
