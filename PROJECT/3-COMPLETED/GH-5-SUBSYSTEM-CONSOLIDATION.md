...
|---|---|
| **Phase O3 + PR2 COMPLETE** (after Phases 4, F, M, R). O3 (PR #17): suppression windows now derived from the doctor freshness registry via one accessor; drift test pins derivation + absolute values (agy: 1 real Blocker — AUTH_RECOVERY unpinned — fixed, then Approved). PR2 (PR #18): one status vocabulary — status axis shares severity's words (FAIL=="error", WARN=="warning"), badge variants/CSS/JS/issue-reporter filter in lockstep; live DB verified free of old literals (no migration); pre-existing swapped badge_html args fixed with proven-red test; tz_utils deleted (plan's "zero references" was stale — four post-plan consumers repointed); token-dependent CI test pinned; vocabulary pin test added. Net −124 lines. agy: 1 real Blocker (reporter tests fed retired vocabulary) — fixed, then Approved. Phase 1b (field collapse) stays deferred per plan. PR #19: runtime __version__ synced (0.69.8) + drift test. | Remaining per plan: wire prior-art-check.sh (parked P-005); Phase 5b chunking (own design pass); name normalisation to subsystem:instance (needs alias map, own slice). The status-vocabulary campaign's executable phases are done. |
