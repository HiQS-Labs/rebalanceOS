...
class GitHubHelperTests(unittest.TestCase):
    ...
    def test_activity_query_helpers(self) -> None:
        with db_connection(self.db_path, ensure_github_schema) as conn:
            ...
            conn.execute(
                "INSERT INTO github_repo_meta (repo_full_name, fetched_at) VALUES (?, date('now'))",
                ("org/hot",),
            )
            conn.commit()
    ...
    def test_artifact_upserts_round_trip(self) -> None:
        with db_connection(self.db_path, ensure_github_schema) as conn:
            ...
            gh.upsert_milestone(conn, (R, 2, "M2", "d", "open", 3, 1, None, None, None, None, "url"))
            gh.upsert_release(conn, (R, 7, "v2", "v2", "main", 0, 1, "notes", None, None, "url"))
            gh.upsert_comment(
                conn,
                (R, "issue", 1, "issue_comment", 50, "me", "OWNER", "hi", "", None, "url", None, None, "f"),
            )
            gh.upsert_link(conn, (R, "pull_request", 9, "issue", 8, "closes"))
            gh.upsert_commit(conn, (R, "pull_request", 9, "sha9", "me", "msg", "c", "url", "f"))
            gh.upsert_check_run(
                conn,
                (R, "pull_request", 9, "sha9", "ci", "completed", "success", "url", None, None, "f"),
            )
            conn.commit()

            self.assertEqual(
                conn.execute("SELECT default_branch, open_issues_count FROM github_repo_meta").fetchone()[
                    "default_branch"
                ],
                "main",
            )
            ...
            self.assertEqual(
                conn.execute("SELECT conclusion FROM github_check_runs").fetchone()["conclusion"],
                "success",
            )
    ...
    def test_upsert_item_binds_by_name(self) -> None:
        with db_connection(self.db_path, ensure_github_schema) as conn:
            ...
            conn.commit()
            row = conn.execute(
                "SELECT item_type, number, title, review_decision, check_status, is_merged FROM github_items"
            ).fetchone()
    ...
    def test_upsert_item_replaces_on_conflict(self) -> None:
        with db_connection(self.db_path, ensure_github_schema) as conn:
            ...
            conn.commit()
            rows = conn.execute("SELECT title FROM github_items WHERE number = 5").fetchall()
    ...
    def test_insert_document_and_delete_item_children(self) -> None:
        with db_connection(self.db_path, ensure_github_schema) as conn:
            ...
            self.assertEqual(conn.execute("SELECT COUNT(*) FROM github_documents").fetchone()[0], 0)
            self.assertEqual(conn.execute("SELECT COUNT(*) FROM github_commits").fetchone()[0], 0)
            self.assertEqual(conn.execute("SELECT COUNT(*) FROM github_items").fetchone()[0], 1)
    ...
    def test_embedding_helpers(self) -> None:
        with db_connection(self.db_path, ensure_github_schema) as conn:
            ...
            self.assertEqual(
                conn.execute("SELECT value FROM github_embedding_meta WHERE key='model_name'").fetchone()["value"],
                "test-model",
            )
            ...
            conn.commit()
            self.assertEqual(conn.execute("SELECT COUNT(*) FROM github_embeddings").fetchone()[0], 0)
    ...
    def test_search_github_documents(self) -> None:
        ...
        with db_connection(self.db_path, ensure_github_schema) as conn:
            ...
            self.assertEqual(len(gh.search_github_documents(conn, vec, 8, repo_full_name=R)), 1)
            self.assertEqual(
    ...
    def test_purge_helpers(self) -> None:
        with db_connection(self.db_path, ensure_github_schema) as conn:
            ...
            self.assertEqual(gh.count_ids_in(conn, "github_embeddings", "doc_id", ids), 1)
            self.assertEqual(gh.count_ids_in(conn, "github_embeddings", "doc_id", []), 0)
