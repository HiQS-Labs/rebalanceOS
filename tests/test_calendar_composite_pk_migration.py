...
_INSERT = f"INSERT INTO calendar_events ({', '.join(_COLS)}) VALUES ({', '.join('?' for _ in _COLS)})"
...
class CalendarCompositePkMigrationTests(unittest.TestCase):
    def _v4_db_with_rows(self, conn, rows: list[tuple]) -> None:
        ...
        conn.execute("INSERT OR IGNORE INTO schema_version (version, applied_at) VALUES (1, '2026-06-01T00:00:00Z')")
        for ver, path in discover_migrations():
            if ver > 4:
                break
            conn.executescript(path.read_text(encoding="utf-8"))
            conn.execute(
                "INSERT OR IGNORE INTO schema_version (version, applied_at) VALUES (?, '2026-06-01T00:00:00Z')", (ver,)
            )
    ...
    def test_data_preserved_with_values_and_teammate_row(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with db_connection(db_path, ensure_schema) as conn:
                ...
                self.assertEqual(
                    conn.execute("SELECT COUNT(*) FROM calendar_events WHERE person IS NOT NULL").fetchone()[0], 0
                )

                # All three rows preserved, incl. the teammate row (copy is NOT
                # implicitly primary-only).
                self.assertEqual(conn.execute("SELECT COUNT(*) FROM calendar_events").fetchone()[0], 3)
                self.assertEqual(
                    conn.execute("SELECT COUNT(*) FROM calendar_events WHERE calendar_id != 'primary'").fetchone()[0], 1
                )

                # Full-tuple readback → columns mapped correctly (no shift/drop).
                got = conn.execute(
                    f"SELECT {', '.join(_COLS)} FROM calendar_events WHERE id='own1' AND calendar_id='primary'"
                ).fetchone()
                ...
                conn.commit()
                self.assertEqual(conn.execute("SELECT COUNT(*) FROM calendar_events WHERE id='own1'").fetchone()[0], 2)
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with db_connection(db_path, ensure_schema) as conn:
                ...
                for cal in ("primary", "teammate@group.calendar.google.com"):
                    conn.execute(_INSERT.replace("INSERT INTO", "INSERT OR REPLACE INTO"), _teammate_row("dup", cal))
                conn.commit()
                cals = [
                    r[0]
                    for r in conn.execute("SELECT calendar_id FROM calendar_events WHERE id='dup' ORDER BY calendar_id")
                ]
    ...
    def test_idempotent(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with db_connection(db_path, ensure_schema) as conn:
                ...
                self.assertEqual(conn.execute("SELECT COUNT(*) FROM calendar_events").fetchone()[0], 1)
                cols = [r[1] for r in conn.execute("PRAGMA table_info(calendar_events)")]
                self.assertIn("person", cols)
