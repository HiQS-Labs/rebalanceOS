...
def test_issue_evidence_preserves_gaps_attempts_and_identity():
    ...
    bundle = dict(prompts=rows, journeys=journeys, parents=parents, orphans=orphans, coverage={}, events=[])
...
def test_issue_evidence_empty_prompts_rejected():
    with pytest.raises(ValueError, match="nonempty"):
        jr.render(dict(prompts=[], journeys=[], parents={}, orphans=[], coverage={}, events=[], issue_evidence=True))
...
def test_issue_evidence_events_are_typed_and_intent_is_redacted():
    ...
    journeys, parents, orphans = jr.group(rows)
    event = dict(
        id="synthetic-issue-event",
        timestamp="2026-09-15T11:00:00Z",
        ref=(jr.REPO, "issue", 568),
        event="closed_at",
        fetched_at="2026-09-16T01:00:00Z",
        url="synthetic-issue-url",
    )
    bundle = dict(
        prompts=rows,
        journeys=journeys,
        ...
        orphans=orphans,
        coverage={},
        events=[event, event | dict(ref=(jr.REPO, "pr", 568), url="wrong-pr-url")],
        issue_evidence=True,
    )
...
@pytest.mark.parametrize(
    ...
    [
        "/start-task #10",
        "/express #10",
        "Please start the fix",
        "Let's start GH10",
        "hotfix: repair loader",
        "[$start-task](skill.md) #10",
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("/start-task on 567 now", "issue", 567),
...
@pytest.mark.parametrize(
    "text", ["567", "phase 2", "version 567.2", "/start-task on PR 2 phase", "issue 626abc", "issue 626.5"]
)
...
def test_qualified_pr_and_issue_do_not_get_duplicate_types():
    assert jr.references("PR #553 and issue #626", True) == [(jr.REPO, "issue", 626), (jr.REPO, "pr", 553)]
...
def test_explicit_links_mode_keeps_uncertainty_out_of_joins():
    raw = (
        json.dumps(
            row("/start-task #10; Catalog PR 8; Other/project#25; https://github.com/HiQS-Labs/XYZ-forge/pull/520")
        ).encode()
        + b"\n"
    )
...
def test_grouping_same_input_and_no_bridge():
    rows, _ = load(
        row(),
        row("continue", timestamp="2026-09-15T11:00:00Z"),
...
def test_qualified_next_task_preserves_original_and_separates_events():
    rows, _ = load(
        row("/start-task https://github.com/HiQS-Labs/XYZ-forge/issues/10"),
        row("Next task: https://github.com/HiQS-Labs/XYZ-forge/issues/20", timestamp="2026-09-15T11:00:00Z"),
    ...
    bundle = dict(
        ...
        events=[
            dict(
                ...
                id="event-20",
                ref=(jr.REPO, "issue", 20),
                event="closed_at",
                url="fixture-close-20",
            )
        ],
    )
...
def test_unclear_transitions_abstain(text):
    rows, _ = load(
        row("/start-task https://github.com/HiQS-Labs/XYZ-forge/issues/10"),
        row(text, timestamp="2026-09-15T11:00:00Z"),
...
def test_transition_needs_a_known_unambiguous_prior_task():
    for initial in ("/start-task now", "/start-task #10 and #30"):
        rows, _ = load(
            row(initial),
            row("Next task: start https://github.com/HiQS-Labs/XYZ-forge/issues/20", timestamp="2026-09-15T11:00:00Z"),
            explicit_links_only=True,
        )
        assert jr.group(rows, qualified_transitions=True) == jr.group(rows)
...
def test_transition_view_excludes_guesses_without_explicit_links_flag():
    ...
    bundle = dict(
        ...
        orphans=original[2],
        coverage={},
        candidate_view=dict(journeys=candidate[0], parents=candidate[1], orphans=candidate[2]),
        events=[
            dict(
                timestamp="2026-09-15T13:00:00Z",
                id="foreign-guess",
...
def test_repeated_start_new_attempt_and_no_lookahead():
    ...
    after, _ = load(
        row(), row("continue", timestamp="2026-09-15T11:00:00Z"), row("/express #10", timestamp="2026-09-15T12:00:00Z")
    )
...
def test_bad_rows_duplicates_and_window():
    rows, counts = load(
        ...
        row(timestamp="bad"),
        row(machine=12),
        row(timestamp="2026-09-01T00:00:00Z"),
        row(repo="unknown", prompt="start #9"),
    )
    assert len(rows) == 1
