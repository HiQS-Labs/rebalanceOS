...
def _card(**over) -> dict:
    base = dict(
        ...
        health_probed_at=_now_iso(minutes=0),
...
class FocusBodyTests(unittest.TestCase):
            ...
            recent_activity=[
                {
                    ...
                    "committed_at": "2026-06-05T00:00:00+00:00",
                    "author_email": "me@x",
                },
            ]
        )
    ...
    def test_html_is_escaped(self) -> None:
        ...
        card = _card(
            newest_pr={
                ...
                "state": "open",
                "html_url": "https://h/pull/1",
                "is_draft": False,
                "is_merged": False,
            },
            recent_activity=[
                {"sha": "d", "subject": "<b>boom</b>", "committed_at": "2026-06-05T00:00:00+00:00", "author_email": "m"}
            ],
        )
        body = _focus5_body(_data([card]))
    ...
    def test_clean_repo_shows_clean_health(self) -> None:
        card = _card(is_dirty=False, modified_count=0, untracked_count=0, rank_reason="your commit 1h ago", ahead=0)
        body = _focus5_body(_data([card]))
    ...
    def test_off_roster_strip_explains_recency_vs_cutoff(self) -> None:
        ...
        warns = [
            {
                "repo_name": "sleuth-app",
                "local_path": "/x/sleuth-app",
                "repo_full_name": None,
                "branch": "main",
                "ahead": 0,
                "modified_count": 1,
                ...
                "probed_at": _now_iso(hours=1),
                "recency_basis": "local_reflog",
                "my_local_commit_ts": now - 3 * 86400,
            }
        ]
        body = _focus5_body(
            _data([_card()], off_roster_warnings=warns, ranking_mode="recent_activity", cutoff=now - 16 * 3600)
        )  # #5 cutoff = 16h ago
    ...
    def test_off_roster_strip_shows_fallback_basis(self) -> None:
        ...
        warns = [
            {
                "repo_name": "no-reflog-clone",
                "local_path": "/x/no-reflog-clone",
                "repo_full_name": None,
                "branch": "main",
                "ahead": 1,
                "modified_count": 0,
                ...
                "is_dirty": False,
                "probed_at": _now_iso(hours=1),
                "recency_basis": "author_email",
                "my_local_commit_ts": now - 2 * 3600,
            }
        ]
        body = _focus5_body(_data([_card()], off_roster_warnings=warns, ranking_mode="recent_activity", cutoff=now))
    ...
    def test_dirty_view_suppresses_focus5_explain_copy(self) -> None:
        ...
        body = _focus5_body(
            _data([_card()], off_roster_warnings=warns, ranking_mode="dirty_first", cutoff=None), view="dirty"
        )
...
class RouteScanTriggerTests(unittest.TestCase):
    ...
    def _load(self, *, refresh: bool, roster_size: int, computed_at):
        ...
        with (
            patch("rebalance.paths.resolve_database_path", return_value="db"),
...
class Focus5GoalsRouteTests(unittest.TestCase):
    ...
    def test_complete_marks_exact_line_then_returns_refreshed_items(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                mock.patch("rebalance.ingest.config.get_vault_path", return_value=tmp),
