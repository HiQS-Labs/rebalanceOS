...
Ported from old-repo PR #305 (GH-5 Phase R) and re-derived against current
source. The live probes route through the shared GitHub client (GH-293): these
tests pin the backwards-stable probe envelopes and the offline funnel verdicts
with an injected fake client — no network. Red pre-port: diagnose had no
``GitHubClient`` symbol to patch, and one 429/5xx produced a confident false
"PAT cannot see this repo" verdict (the observed user-facing bug).
"""
...
class LiveProbeTests(unittest.TestCase):
    ...
    def test_commit_probe_survives_an_empty_commit_message(self) -> None:
        ...
        fake = _client_cls(result={"sha": "abc123", "commit": {"message": "", "committer": {}}})
        with patch.object(diagnose, "GitHubClient", fake):
            out = diagnose._live_probe_commit("someone/repo", "abc123", "tok")
        self.assertTrue(out["exists"])
...
class DiagnoseRepoFunnelTests(unittest.TestCase):
    ...
    def test_live_pat_failure_surfaces_first_next_action(self) -> None:
        ...
        with (
            patch.object(diagnose, "get_github_token", return_value="tok"),
