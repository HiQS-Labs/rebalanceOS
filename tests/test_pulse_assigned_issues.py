...
source. fetch_assigned_issues routes through the shared GitHub client; these
tests pin the row mapping and the rate-limit error contract with an injected
fake — no network. The old code string-matched "rate limit" in the error body;
the shared client detects it from response headers, error contract unchanged.
"""
