...
def _raw_gather_team_activity(
    ...
    cutoff = time_ops.now_utc() - timedelta(minutes=minutes)
...
def _raw_gather_unwatched_active_repos(
    ...
    cutoff = time_ops.now_utc() - timedelta(days=fresh_threshold_days)
...
def _raw_gather_snapshot(login: str, token: str, db_path: Path, minutes: int, top_n: int) -> dict[str, Any]:
    ...
    return {
        "raw_version": 1,
        "scanned_at": time_ops.now_utc().isoformat(timespec="seconds"),
        "login": login,
