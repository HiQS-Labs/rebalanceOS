...
3. **You authored in it in the last 14 days** — a commit, a push, or a pull request
   opened or merged. One event is enough.
4. **You pushed to it in the last 14 days** per GitHub's own pushed-repos list, which
   catches pushes the events feed misses.
...
- it ages out — if activity or a push was its only reason to be monitored, it leaves
  the set silently 14 days after the last one. There is no other drop mechanism, and
  none is planned: your own activity defines the set.

**A repo becomes a project on its own** once you have personally authored 3 or more
commits to it (all-time, threshold configurable). The promoted row is machine-owned and
never overwrites a curated project of the same name. See "Auto-promotion" in
`AGENTS.md`.
