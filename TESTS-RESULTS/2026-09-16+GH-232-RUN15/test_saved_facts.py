...
def delivery_snapshot():
    value = snapshot()
    value['prs'] = [dict(number=20, url='https://github.com/HiQS-Labs/XYZ-forge/pull/20',
                         mergedAt='2026-09-15T12:00:00Z', state='MERGED',
                         closingIssuesReferences=[dict(number=10,
...
@pytest.mark.parametrize('mutation', ['wrong_number', 'wrong_kind', 'foreign_repository',
                                    'conflicting_repository', 'cutoff', 'unmerged', 'missing_refs'])
...
def test_delivery_render_preserves_original_and_compares_same_facts():
    ...
    links, gaps = FACTS.saved_delivery_links(JR, value, AS_OF)
    updated = bundle | dict(issue_evidence=True, delivery_links=links, delivery_gaps=gaps,
                            review_cases=[10])
