...
def artifact_identity(jr, item, kind):
    ...
    url = item.get('url')
    if (not isinstance(url, str) or not jr.REF.fullmatch(url)
...
def saved_delivery_links(jr, snapshot, as_of):
    ...
    for item in snapshot['prs']:
        ...
        if key is None:
            ...
            continue
        signature = _json_dumps(dict(mergedAt=item.get('mergedAt'), state=item.get('state'),
    ...
    for pr, duplicates in sorted(records.items()):
        ...
        for target in targets:
            ...
            links[lid] = dict(id=lid, pr=pr, issue=issue, merged_at=merged.isoformat(),
                              fetched_at=snapshot['retrieved_at_utc'], source_field='closingIssuesReferences',
                              pr_url=item['url'], issue_url=target['url'])
