...
def bundles():
    original = dict(source={'sha256': 'fixture', 'bytes': 1}, as_of_utc='frozen',
                    prompts=[dict(id='fixture-1', ordinal=1, text='issue 12', start=True, refs=[])],
                    journeys=[dict(id='fixture-1', prompts=['fixture-1'])], orphans=[],
                    coverage=dict(eligible_prompts=1, starts=1, journeys=1, orphans=0, devices=1,
                                  scanned=1, out_of_window=0, malformed=0,
                                  unresolved_or_other_repo=0, byte_cap_hit=False))
