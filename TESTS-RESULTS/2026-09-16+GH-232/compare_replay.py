...
def compare(baseline, before, after):
    ...
    def identity(bundle):
        return [{key: value for key, value in prompt.items() if key != "refs"} for prompt in bundle["prompts"]]

    assert identity(baseline) == identity(before) == identity(after), "prompt identity/start drift"

    def membership(bundle):
        return [(journey["id"], journey["prompts"]) for journey in bundle["journeys"]]
    ...
    for key in (
        ...
        "out_of_window",
        "malformed",
        "unresolved_or_other_repo",
        "byte_cap_hit",
    ):
        assert baseline["coverage"].get(key) == before["coverage"].get(key) == after["coverage"].get(key), key
    records, private = (
        [],
        ["# Private reference review", "", "Assistant diagnostic review, not human accuracy labels.", ""],
    )
    for original, previous, current in zip(baseline["prompts"], before["prompts"], after["prompts"]):
        ...
        records.append(
            dict(
                case=case,
                prior_changed=old != prev,
                changed_now=prev != new,
                added=len(new - prev),
                removed=len(prev - new),
                retained=len(prev & new),
                retained_prior_additions=len((prev - old) & new),
                final_refs=len(new),
            )
        )
        private += [
            f"## Case {case} — source ordinal {current['ordinal']}",
            f"ID: {current['id']}",
            f"Original: {sorted(old)}",
            f"Before: {sorted(prev)}",
            f"After: {sorted(new)}",
            "",
            current["text"],
            "",
...
def main():
    ...
    print(
        _json_dumps(
            dict(
                review_cases=len(records),
                prior_cases=sum(r["prior_changed"] for r in records),
