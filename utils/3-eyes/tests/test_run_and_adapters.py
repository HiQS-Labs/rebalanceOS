...
def test_emitted_finding_is_routed(activate, monkeypatch):
    ...
    (emit / "selfcheck.json").write_text(
        json.dumps({"title": "found something", "severity": "warn", "summary": "s", "text": "t"})
    )
    routed = []
...
def test_observe_existing_reads_and_tags(monkeypatch, tmp_path):
    ...
    with open(agents_dir / "com.rebalance-os.3eyes.selfcheck.plist", "wb") as fh:
        plistlib.dump(
            {"Label": "com.rebalance-os.3eyes.selfcheck", "ProgramArguments": ["/bin/echo"], "StartInterval": 3600}, fh
        )
