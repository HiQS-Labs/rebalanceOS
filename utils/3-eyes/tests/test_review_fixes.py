...
def test_classify_skipped_when_budget_exhausted(activate, monkeypatch):
    ...
    monkeypatch.setattr(classify, "classify", lambda *a, **k: pytest.fail("classify called despite exhausted budget"))
...
def test_zero_route_finding_is_deadlettered_not_deleted(activate):
    ...
    activate()
    job = registry.Job(id="noroute", command="noop", routes=(), schedule={"launchd": {"StartInterval": 60}})
    emit = config.state_dir() / "emit"
    emit.mkdir(parents=True, exist_ok=True)
...
def test_disabled_job_is_skipped(activate, monkeypatch):
    activate()
    disabled = registry.Job(id="off", command="noop", enabled=False, schedule={"launchd": {"StartInterval": 60}})
    monkeypatch.setattr(registry, "load_job", lambda job_id, **k: disabled)
    monkeypatch.setattr(breakers, "run_job_command", lambda job: pytest.fail("ran a disabled job"))
